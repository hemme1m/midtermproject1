...
$("#submitBtn").click(function() {
	$("#" + reservedSeat).css("backgroundColor", "red")	//upon submission, changes the color of the reserved seat box
	...
	console.log(reservations);					//per David's request, logs reservations array to console
});
...
$(".box").mouseover(function() {
	...
	for (var i = 0; i < reservations.length; i++) {
		if(hoveredSeat === reservations[i].seat) {		//while for loop is running, checks to see if there is a match between hovered seat id and seat id for an object in the array
			$(this).attr("title" , reservations[i].name + ", " + reservations[i].email + ", " + reservations[i].seat ); //if seat is reserved, this changes the title for that element to user name, email and seat number
		}
	}
});
